...
class ManagedIdentitiesServiceClientMeta(type):
    ...
    objects.
    """
    _transport_registry = OrderedDict()  # type: Dict[str, Type[ManagedIdentitiesServiceTransport]]
...
class ManagedIdentitiesServiceClient(metaclass=ManagedIdentitiesServiceClientMeta):
    ...
    def domain_path(project: str,location: str,domain: str,) -> str:
        ...
        return "projects/{project}/locations/{location}/domains/{domain}".format(project=project, location=location, domain=domain, )

    @staticmethod
    def parse_domain_path(path: str) -> Dict[str,str]:
        """Parse a domain path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/domains/(?P<domain>.+?)$", path)
    ...
    def create_microsoft_ad_domain(self,
        ...
        rpc = self._transport._wrapped_methods[self._transport.create_microsoft_ad_domain]
